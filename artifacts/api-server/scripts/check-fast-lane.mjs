 ...
 *
 * Runs against a THROWAWAY database: it must be executed from a temp cwd, since
 * db.ts resolves `data/` relative to the working directory. run-tests.mjs does
 * this for you.
 */
...
const prov = foldProvisionalPublishers(sd.rawPlayListDeveloperGroups());
...
rollupPublishers(() => {});
const beforeFast = sd.listPublishersByScore(9999).filter((p) => p.name === 'FastLane Studios');
...
rollupProvisionalPlayPublishers(() => {});
const afterFast = sd.listPublishersByScore(9999).filter((p) => p.name === 'FastLane Studios');
...
for (let i = 1; i <= 3; i++) {
  raw.prepare(`INSERT OR REPLACE INTO store_app_detail
...
rollupPublishers(() => {});
const afterFull = sd.listPublishersByScore(9999).filter((p) => p.name === 'FastLane Studios');
afterFull.length === 1
...
rollupProvisionalPlayPublishers(() => {});
const afterRe = sd.listPublishersByScore(9999).filter((p) => p.name === 'FastLane Studios');
...
raw.prepare("DELETE FROM store_app_detail WHERE app_id LIKE 'com.fl.app%'").run();
raw.prepare("DELETE FROM publishers WHERE name='FastLane Studios'").run();
raw.prepare("DELETE FROM publisher_identity").run();
...
resetFastLaneCache();
rollupProvisionalPlayPublishers(() => {});
const q = sd.listPublishersForConfirmation().filter((p) => p.name === 'FastLane Studios');
...
const folded = foldProvisionalPublishers(sd.rawPlayListDeveloperGroups())
...
rollupProvisionalPlayPublishers(() => {});          // absorb the spelling rows
const noop = rollupProvisionalPlayPublishers(() => {}); // now nothing has changed
...
const after = rollupProvisionalPlayPublishers(() => {});
after.publishers > 0
...
//    which silently dropped tail publishers out of the confirmation queue.
{
  for (const id of ['com.pe.a1', 'com.pe.a2']) {
    sd.upsertDiscoveredApp({ store: 'google_play', app_id: id, title: 'PE', vertical: 'games',
  ...
  rollupPublishers(() => {});
  const b = sd.listPublishersByScore(9999).find((p) => p.name === 'Preserve Games');
  // A new, still-unenriched chart app arrives for the same publisher.
  sd.upsertDiscoveredApp({ store: 'google_play', app_id: 'com.pe.a3', title: 'PE3', vertical: 'games',
    country: 'us', source: 'chart', chart: 'TOP_FREE', rank: 4, list_developer: 'Preserve Games' });
  resetFastLaneCache();
  rollupProvisionalPlayPublishers(() => {});
  const a = sd.listPublishersByScore(9999).find((p) => p.name === 'Preserve Games');
...
{
  sd.upsertDiscoveredApp({ store: 'google_play', app_id: 'com.pvw.decoy1', title: 'Decoy 1',
    vertical: 'finance', country: 'us', source: 'search', list_developer: 'Preview Pick Co' });
  sd.upsertDiscoveredApp({ store: 'google_play', app_id: 'com.pvw.mid', title: 'Mid',
  ...
  sd.upsertDiscoveredApp({ store: 'google_play', app_id: 'com.pvw.best', title: 'Best',
    vertical: 'finance', country: 'us', source: 'chart', chart: 'TOP_FREE', rank: 3, list_developer: 'Preview Pick Co' });
  const g = sd.rawPlayListDeveloperGroups().find((r) => r.developer === 'Preview Pick Co');
  ...
  sd.upsertDiscoveredApp({ store: 'google_play', app_id: 'com.other.zz', title: 'ZZ',
    vertical: 'finance', country: 'us', source: 'search', list_developer: 'Unrelated Co' });
  const g2 = sd.rawPlayListDeveloperGroups().find((r) => r.developer === 'Preview Pick Co');
